...
def main():
    ...
    is_file_and_image = False
    exists = False
    try:
        if input_path.is_file() and is_image_file(input_path):
            is_file_and_image = True
        elif input_path.exists():
            exists = True
    except Exception:
        pass

    if is_file_and_image:
        img = Image.open(args.input).convert('L')
    elif not exists:
        eprint("Treating input as text...")
        ...
        with tempfile.NamedTemporaryFile(delete=False, suffix='.png') as temp_file:
            ...
            qr.add_data(open(args.input, 'r').read())
            qr.make(fit=True)
    ...
    eprint("Outputting QRcode as Unicode characters.")
    if args.output:
        with open(args.output, 'w') as file:
            for row in qr_blocks:
                for block in row:
                    # Using two spaces for white blocks
                    file.write(black_block if block == 1 else white_block)
                file.write('\n')
    else:
        ...
        for row in qr_blocks:
            for block in row:
                # Using two spaces for white blocks
                print(black_block if block == 1 else white_block, end='')
            print()
